...
class DiscussionList extends Component {
  ...
  allMessages = (messages) =>
    messages.map((message) => {
      return (
        <div>
          <DiscussionMessage key={`Message-${String(new Date())}`} msg={message} />
        </div>
      );
    });
  ...
  componentWillUnmount() {
    // this clears the interval when you go on another page
...
export default class Discussion extends Component {
  ...
  // remember, we can use deconstruction in react to get the state
  render() {
